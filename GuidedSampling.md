...
The insight is clean: **RS implicitly combines exploration and generation, and it's bad at exploration. GuidedSampling makes exploration explicit.** That's it. The whole paper follows from that one observation.

What I find most interesting is the post-training result — not just that it works, but *why* it generalizes out-of-domain. Training on diverse MATH reasoning chains improves GPQA-Diamond performance. This suggests the diversity itself is the transferable thing, not the specific mathematical content. That's a hint that concept-diverse data might be a general-purpose training signal worth pursuing more seriously.

The Qwen failure case is actually the most instructive result in the paper. A method that fails in a principled, explainable way (concept generator is too weak → no diversity → no benefit) is far more trustworthy than one that just always works. It tells you exactly where the method's assumptions break down.

---

*Summary written as part of the 45-Day Hard research plan. Format: Problem → Prior Failure → Key Innovation → Method → Experiments → Limitations → Open Questions.*
